...
function(add_split_configured_source target scope file_name variables values defines counter)
    set(ctr ${${counter}})
    if (variables)
        ...
        foreach (val ${var_values})
            set(next_defines "${defines}")
            list(APPEND next_defines "#define ${var_name} ${val}")
            add_split_configured_source("${target}" "${scope}" "${file_name}" "${variables}" "${values}"
                    "${next_defines}" ctr)
        endforeach ()
    else ()
        ...
        set(gen_lines "${defines}")
        list(APPEND gen_lines "#include \"${CMAKE_CURRENT_SOURCE_DIR}/${file_name}\"" "")
        list(JOIN gen_lines "\n" gen_content)
...
function(target_split_configured_sources target scope)
    ...
    foreach(arg ${ARGN})
        ...
        elseif (state STREQUAL "KEY_VALUES")
            if (arg STREQUAL "VALUES")
                ...
                message(FATAL_ERROR "Expected VALUES")
            endif ()
        elseif (state STREQUAL "VALUES")
            ...
            message(FATAL_ERROR "?? state ${state}")
        endif ()
    endforeach()
    if (state STREQUAL "VALUES")
        list(LENGTH values next_value_idx)
        list(APPEND variables "${var_name}" "${first_value_idx}" "${next_value_idx}")
    elseif (NOT state STREQUAL "FILE_NAME")
    ...
    foreach (file_name ${file_names})
        set(ctr 0)
        add_split_configured_source("${target}" "${scope}" "${file_name}" "${variables}" "${values}"
                "#define SPLIT_CONFIGURATION 1" ctr)
    endforeach ()
endfunction()
