...
class stdio_input_stream final
    : public input_stream {
    public:
        explicit stdio_input_stream(const std::string &file_name, size_t chunk_size)
            : _chunk_size(chunk_size)
            , _chunk(malloc(chunk_size)) {
            if (!file_name.empty() && file_name != "-") {
                _file = fopen(file_name.c_str(), "rb");
                if (!_file) {
                    throw io_error("fopen: " + file_name + ": " + strerror(errno));
                }
            } else {
                _file = freopen(nullptr, "rb", stdin);
                if (!_file) {
                    throw io_error("freopen: stdin: "s + strerror(errno));
                }
            }
        }
        ...
        ~stdio_input_stream() noexcept(false) override {
            ...
            if (_file != stdin) {
                if (fclose(_file) != 0) {
                    throw io_error("fclose: "s + strerror(errno));
                }
            }
        }
    ...
    public:
        explicit stdio_output_stream(const std::string &file_name, size_t max_chunk_length)
            ...
            if (!file_name.empty() && file_name != "-") {
                ...
                if (!_file) {
                    throw io_error("freopen: stdout: "s + strerror(errno));
                }
            }
        }
...
class mmap_input_stream final
    ...
    public:
        ...
        ~mmap_input_stream() noexcept(false) override {
            ...
            if (munmap_result == -1) {
                throw io_error("munmap: input: "s + strerror(errno));
            }
            if (close_result == -1) {
                throw io_error("close: input: "s + strerror(errno));
            }
        }
...
class mmap_output_stream final
    : public output_stream {
    public:
    ...
    private:
        ...
        void unmap_if_mapped() {
            if (_map) {
                if (munmap(_map, _capacity) == -1) {
                    throw io_error("munmap: "s + strerror(errno));
                }
            }
            _map = nullptr;
        }
...
stdio_io_factory::create_output_stream(const std::string &file_name, size_t max_chunk_length) const {
...
std::unique_ptr<output_stream>
mmap_io_factory::create_output_stream(const std::string &file_name, size_t max_chunk_length) const {
