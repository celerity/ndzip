...
TEST_CASE("Warp-cooperative primitives work", "[cuda][scan]") {
    ...
    SECTION("warp_reduce") {
        ...
        CHECKED_CUDA_CALL(cudaMemcpy, out.data(), buf.get(), 2 * warp_size * sizeof(index_type),
    ...
    SECTION("warp_inclusive_scan") {
        ...
        CHECKED_CUDA_CALL(cudaMemcpy, out.data(), buf.get(), 2 * warp_size * sizeof(index_type),
                cudaMemcpyDeviceToHost);
        iter_inclusive_scan(
                iota.begin(), iota.begin() + warp_size, ref.begin(), plus<index_type>{});
        iter_inclusive_scan(iota.begin() + warp_size, iota.begin() + 2 * warp_size,
                ref.begin() + warp_size, plus<index_type>{});
        CHECK_FOR_VECTOR_EQUALITY(out, ref);
    }
    ...
    SECTION("warp_exclusive_scan") {
        ...
        iter_exclusive_scan(
                iota.begin(), iota.begin() + warp_size, ref.begin(), plus<index_type>{});
        iter_exclusive_scan(iota.begin() + warp_size, iota.begin() + 2 * warp_size,
                ref.begin() + warp_size, plus<index_type>{});
        CHECK_FOR_VECTOR_EQUALITY(out, ref);
    }
}
