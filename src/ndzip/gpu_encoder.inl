...
size_t ndzip::gpu_encoder<T, Dims>::compress(
    ...
    _pimpl->q.submit([&](sycl::handler &cgh) {
        ...
        auto compressed_blocks_acc
                = compressed_blocks_buffer.template get_access<sycl::access::mode::discard_write>(
                        cgh);
        auto compressed_block_lengths_acc
                = compressed_block_lengths_buffer.get_access<sycl::access::mode::discard_write>(
                        cgh);

        // local memory
        auto block_transform_acc = sycl::accessor<bits_type, 1,
                sycl::access::mode::discard_read_write, sycl::access::target::local>{hc_size, cgh};
        auto block_compaction_acc
                = sycl::accessor<bits_type, 1, sycl::access::mode::discard_read_write,
